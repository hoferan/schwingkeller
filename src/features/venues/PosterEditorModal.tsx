  ...
  const toggle = (key: string, text: string, checked: boolean, set: (v: boolean) => void) => (
    <label key={key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', cursor: 'pointer', fontSize: '14.5px', color: theme.color.ink, fontWeight: 500 }}>
      ...
      <span style={{ position: 'relative', width: '46px', height: '28px', borderRadius: '999px', background: checked ? theme.color.accent : theme.color.line, transition: 'background .18s ease', flex: 'none' }}>
        ...
        {/* pointerEvents none so the knob does not sit in front of the checkbox it decorates.
            Clicks already reached the input through the wrapping label, but anything aiming at the
            input itself — a browser test, an automation tool — hit this span instead. */}
        <span style={{ position: 'absolute', top: '3px', left: checked ? '21px' : '3px', width: '22px', height: '22px', borderRadius: '50%', background: '#fff', boxShadow: '0 1px 3px rgba(0,0,0,.35)', transition: 'left .18s ease', pointerEvents: 'none' }} />
      </span>
    </label>
  );

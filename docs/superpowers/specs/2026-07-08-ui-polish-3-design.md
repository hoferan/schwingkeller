...
- `onTouchEnd` (JSX prop, as today): if the gesture never became a drag (`isDraggingRef.current` was `false` throughout — i.e. it was either a pass-through list scroll or too small/never moved), fall back to the existing tap-toggle check, but only when `startedOnHeaderRef.current` is `true`: if `|finalDeltaY| < 10`, call `e.preventDefault()` (suppressing the compat click, as today) and `onToggleSidebar()`. If the gesture *was* a drag, call `setDragHeight(null)` (releasing manual control so the CSS transition takes over) and decide the target state relative to where the drag *started*, not the sheet's absolute midpoint: let `range = openHeightPxRef.current - 108` and `travelled = lastDragHeight - dragStartHeightRef.current`; if `travelled > range * 0.25`, call `onSetSidebarOpen(true)` (dragged up far enough to commit to opening); if `travelled < -range * 0.25`, call `onSetSidebarOpen(false)` (dragged down far enough to commit to closing); otherwise call `onSetSidebarOpen(sidebarOpen)` (snap back to whichever state the drag started from — the drag didn't go far enough to commit to the other one).

  (Revision note: the spec originally called for snapping to whichever of the two fixed endpoints is numerically closer across the *whole* 108px–~600px+ range — that requires dragging past the geometric midpoint, i.e. 250px+ of travel, to ever flip state, which is a far heavier gesture than the old 30px swipe threshold and would feel broken. The 25%-of-range-from-the-starting-point rule above preserves a quick, decisive swipe while still tracking the finger continuously during the drag.)
...
**Explicitly out of scope:** no velocity/fling-based snapping (purely position-relative-to-drag-start at release, per the revision above); no visual "rubber-banding" past the peek/open bounds (hard clamp instead); no changes to the click-outside-to-close effect (unrelated, untouched).
